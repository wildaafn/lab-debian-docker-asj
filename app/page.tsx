...
export default function Home() {
  ...
  function saveList(setter: React.Dispatch<React.SetStateAction<string[]>>, key: string, update: (prev: string[]) => string[]) {
    setter((prev) => { const next = update(prev); localStorage.setItem(key, JSON.stringify(next)); return next; });
  }
  ...
  const filtered = useMemo(() => modules.filter((m) => `${m.title} ${m.summary} ${m.objectives.join(" ")}`.toLowerCase().includes(query.toLowerCase())), [query]);
  ...
  return <div className="app-shell">
    {celebrate && <div className="celebration" role="status" aria-live="polite"><div className="confetti">{Array.from({ length: 18 }).map((_, i) => <i key={i} />)}</div><strong>Modul selesai! +100 XP</strong><span>Server berikutnya menunggu.</span></div>}
    ...
    <main className="main">
      <header className="topbar"><button className="menu-button" onClick={() => setMenu(true)} aria-label="Buka menu">☰</button><div className="crumb"><span>Administrasi Sistem Jaringan</span><b>/</b><strong>{active === "overview" ? "Ringkasan" : modules.find((m) => m.id === active)?.title}</strong></div><div className="platforms"><span>Kelas XI</span><span>Kelas XII</span><span>Docker</span></div></header>
      <div className="content">
        {active === "overview" ? <>
          <section className="hero">
            <div className="hero-copy"><span className="eyebrow">Kurikulum ASJ • Docker lab • CLI server</span><h1>Belajar ASJ mendalam<br/><em>dari konsep ke praktik.</em></h1><p>Lab interaktif untuk ASJ XI-XII TKJ. Disusun oleh Wilda Ariffatul Faisalnur, S.Kom. Materi mencakup OS jaringan, DHCP, remote server, DNS, FTP, web, database, mail, proxy, control panel hosting, keamanan, dan troubleshooting.</p><div className="hero-actions"><button className="primary-button" onClick={() => go("persiapan")}>Mulai praktik <span>→</span></button><a className="secondary-button" href="#kurikulum">Lihat kurikulum</a></div></div>
          ...
          <section className="stats"><div><strong>{modules.length}</strong><span>Modul bertahap</span></div><div><strong>XI-XII</strong><span>Alur silabus ASJ</span></div><div><strong>CLI</strong><span>Mode server utama</span></div><div><strong>1</strong><span>Jaringan terisolasi</span></div></section>
          <section className="mission-grid">
            <div className="mission-card mission-main"><div><span className="eyebrow">Lanjutkan petualangan</span><h2>{resumeModule.title}</h2><p>{completed.length === 0 ? "Mulai dari konsep Docker dan OS jaringan sebelum masuk layanan server." : "Progresmu tersimpan di perangkat ini. Lanjut tepat dari tempat terakhir."}</p></div><button className="primary-button" onClick={() => go(resumeModule.id)}>Buka modul <span>→</span></button></div>
            ...
            <div className="mission-card rank-card"><div className="rank-ring" style={{ "--rank": `${levelProgress * 3.6}deg` } as React.CSSProperties}><span>{level}</span></div><div><span className="eyebrow">Level teknisi</span><h3>{xp < 500 ? "CLI Beginner" : xp < 1000 ? "Server Operator" : "Network Admin"}</h3><p>{250 - (xp % 250)} XP menuju level berikutnya</p></div></div>
          </section>
          <section className="overview-section" id="topologi"><div className="section-intro"><span className="eyebrow">Arsitektur praktik</span><h2>Satu jaringan, layanan terpisah</h2><p>Kerusakan satu container tidak menghapus seluruh pekerjaan. Semua layanan diakses oleh Debian client melalui jaringan <code>lab-asj</code>.</p></div><div className="topology"><div className="topology-client"><Icon name="terminal"/><strong>client-asj</strong><span>172.25.0.10</span></div><div className="topology-line"><span>lab-asj • 172.25.0.0/24</span></div><div className="server-grid">{topologyModules.map((m) => <button key={m.id} onClick={() => go(m.id)}><Icon name={m.icon}/><strong>{m.title.replace(" Server", "")}</strong><span>{({web:".80",database:".100",ftp:".21",dns:".53",mail:".25",dhcp:"simulasi"} as Record<string,string>)[m.id]}</span></button>)}</div></div></section>
          <section className="module-list" id="kurikulum"><div className="section-intro"><span className="eyebrow">Kurikulum mendalam</span><h2>Pilih modul pembelajaran</h2></div><div className="search"><span>⌕</span><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Cari DHCP, SSH, DNS, proxy..." /></div><div className="cards">{filtered.map((m) => <button className="module-card" key={m.id} onClick={() => go(m.id)}><div className="card-top"><span className="card-icon"><Icon name={m.icon}/></span><span className="module-no">{m.number}</span></div><h3>{m.title}</h3><p>{m.summary}</p><div className="card-meta"><span>{m.duration}</span><span>{m.level}</span>{completed.includes(m.id) && <span className="complete">✓ Selesai</span>}</div></button>)}</div></section>
          <section className="safety"><div><span className="eyebrow">Wajib dibaca</span><h2>Praktik aman sebelum jaringan nyata</h2><p>Docker Desktop di macOS dan Windows berjalan melalui VM/NAT. Materi DHCP, proxy, mail, dan DNS dibuat untuk jaringan lab terisolasi sebelum diterapkan pada server sekolah yang sebenarnya.</p><button className="secondary-button light" onClick={() => go("security-troubleshooting")}>Buka keamanan →</button></div><div className="safety-icon"><Icon name="shield" size={58}/></div></section>
      ...
      <footer><span>Lab Debian Docker • ASJ XI-XII TKJ</span><span>Wilda Ariffatul Faisalnur, S.Kom • SMKS Islam 1 Kota Blitar</span></footer>
    </main>
  </div>;
}
